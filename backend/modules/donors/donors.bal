...
export const BloodDonationEligibilityResult: React.FC = () => {
  ...
  const { isEligible, result, gender, failedCriteria } = (location.state as LocationState) || {
    isEligible: false,
  ...
  const handleConfirmDonation = () => {
    ...
    alert("Thank you for choosing to donate! Please contact a donation center to schedule your appointment.");
  };
          ...
          async (position) => {
            ...
            try {
              ...
              if (response.data.length === 0) {
                setLocationError("No donation centers found within 10 km.");
              }
            } catch (err: any) {
              ...
              if (err.response) {
                if (err.response.status === 401) {
                  ...
                  errorMessage = "Donation centers endpoint not found on server.";
                } else if (err.response.status === 400) {
                  errorMessage = "Invalid request: Check location parameters.";
                } else {
                  errorMessage = `Failed to fetch donation centers: ${err.response.data?.message || err.message} (Status: ${err.response.status})`;
                }
              } else if (err.request) {
          ...
          {isEligible && (
            <div className="mt-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">
                Nearby Donation Centers (within 10 km)
              </h2>
              {locationError && (
              ...
              {!loadingCenters &&
                ...
                donationCenters.length === 0 && (
                  <p className="text-gray-600 text-sm mb-4">
                    No donation centers found within 10 km. Try checking with a local blood bank.
                  </p>
                )}
              {userLocation && (
                <p className="text-gray-600 text-sm mb-4">
                        ...
                        {donationCenters.map((center) => (
                          ...
                          >
                            ...
                            <p className="text-sm text-gray-600">
                              Address: {center.hospital_address || "Not provided"}
                            </p>
                            <p className="text-sm text-gray-600">
