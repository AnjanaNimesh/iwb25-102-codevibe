...
//     // Protected: Get all blood requests for a hospital
//     resource function get bloodrequests(http:Request req) returns http:Response|error {
//         // Validate JWT token from cookie and check role
//         http:Response? authResult = validateHospitalUserRole(req);
//         if authResult is http:Response {
//             return authResult; // Return error response if validation fails
//         }

//         string? hospitalIdStr = req.getQueryParamValue("hospital_id");
//         if hospitalIdStr is () {
//             http:Response response = new;
//             response.setJsonPayload({ status: "error", message: "Missing hospital_id in request" });
...
//         if hospitalIdResult is error {
//             http:Response response = new;
//             response.setJsonPayload({ status: "error", message: "Invalid hospital_id format" });
//             response.statusCode = 400;
//             return response;
//         }
...
//     // Protected: Get single blood request by ID
//     resource function get bloodrequests/[int requestId](http:Request req) returns http:Response|error {
//         // Validate JWT token from cookie and check role
//         http:Response? authResult = validateHospitalUserRole(req);
//         if authResult is http:Response {
//             return authResult; // Return error response if validation fails
...
// // Function to validate JWT token from cookie and check hospital_user role
// function validateHospitalUserRole(http:Request req) returns http:Response|() {
//     // Get JWT token from cookie
//     http:Cookie[] cookies = req.getCookies();
//     string? token = ();
    
//     foreach http:Cookie cookie in cookies {
//         if cookie.name == "hospital_auth_token" {
//             token = cookie.value;
//             break;
//         }
//     }
...
//         signatureConfig: {
//             secret: "your-secret-key-here-make-it-long-and-secure" // Must match the secret used in auth_service.bal
//         }
//     };
...
//         io:println("JWT validation error: ", result.message());
//         http:Response response = new;
//         response.setJsonPayload({ 
//             status: "error", 
...
//     // Check if role claim exists and equals "hospital_user" using member access
//     anydata roleResult = payload["role"];
...
//     string|error roleStr = roleResult.ensureType(string);
//     if roleStr is error || roleStr != "hospital_user" {
//         http:Response response = new;
//         response.setJsonPayload({ 
//             status: "error", 
...
//     // Check token expiration - use member access and convert int to decimal
//     anydata expTimeData = payload["exp"];
//     if expTimeData is () {
//         http:Response response = new;
//         response.setJsonPayload({ 
//             status: "error", 
...
//     int|error expTimeInt = expTimeData.ensureType(int);
//     if expTimeInt is error {
//         http:Response response = new;
//         response.setJsonPayload({ 
//             status: "error", 
...
//     decimal expTime = <decimal>expTimeInt;
...
//             return userResult; // Return error response if validation fails
//         }
...
//             SELECT hospital_email as user_id, hospital_email as email, hospital_email as full_name, hospital_id, 'hospital_user' as role, status
//             FROM hospital_user
...
//         // Get hospital_id from database using user's email (since JWT doesn't have hospital_id)
...
//         check hospitalStream.forEach(function(record {int hospital_id;} row) {
//             hospitalResults.push(row);
//         });

//         if hospitalResults.length() == 0 {
//             http:Response response = new;
//             response.setJsonPayload({ status: "error", message: "User not found or not associated with any hospital" });
//             response.statusCode = 403;
//             return response;
//         }

//         int hospitalId = hospitalResults[0].hospital_id;

//         stream<BloodRequest, sql:Error?> resultStream = database:dbClient->query(`
//             SELECT br.request_id, br.hospital_id, br.admission_number, br.blood_group, 
//                    br.units_required, br.request_date, br.request_status, br.notes
//             FROM blood_request br
//             WHERE br.hospital_id = ${hospitalId} AND br.is_deleted = 0
...
//                    br.units_required, br.request_date, br.request_status, br.notes
//             FROM blood_request br
//             WHERE br.request_id = ${requestId} AND br.hospital_id = ${hospitalId} AND br.is_deleted = 0
//         `);
...
//         // Check if user has permission to update this request (optional: only creator or admin can update)
//         stream<BloodRequest, sql:Error?> checkStream = database:dbClient->query(`
//             SELECT created_by_user_id FROM blood_request 
//             WHERE request_id = ${requestId} AND hospital_id = ${hospitalId} AND is_deleted = 0
//         `);

//         BloodRequest[] existingRequests = [];
//         check checkStream.forEach(function(BloodRequest row) {
...
//         sql:ExecutionResult result = check database:dbClient->execute(`
//             INSERT INTO blood_request 
//                 (hospital_id, blood_group, units_required, request_status, notes, admission_number, 
//                  is_deleted, created_by_user_id)
//             VALUES (${hospitalId}, ${requestData.blood_group}, ${requestData.units_required}, 
//                     ${requestData.request_status}, ${requestData.notes}, ${requestData.admission_number}, 
//                     0, ${userData.user_id})
//         `);
...
//     // Protected: Get user's own blood requests
//     resource function get my\-requests(http:Request req) returns http:Response|error {
//         // Extract user data from JWT token
//         UserPayload|http:Response userResult = extractUserFromToken(req);
//         if userResult is http:Response {
//             return userResult; // Return error response if validation fails
...
//             FROM blood_request br
//             WHERE br.created_by_user_id = ${userData.user_id} AND br.is_deleted = 0
//             ORDER BY br.request_date DESC
//         `);
...
// // Enhanced function to extract user data from JWT token
// function extractUserFromToken(http:Request req) returns UserPayload|http:Response {
//     // Get JWT token from cookie - try both cookie names for compatibility
//     http:Cookie[] cookies = req.getCookies();
//     string? token = ();
...
//     // Parse the simple JWT token (no signature validation since it's "alg": "none")
...
//     // Extract hospital_id if present (could be in userId for hospital users)
//     int? hospitalId = ();
//     if userId != "" {
...
// function validateHospitalUserRole(http:Request req) returns http:Response|() {
//     UserPayload|http:Response result = extractUserFromToken(req);
...
//     // Add this endpoint to your hospital service
