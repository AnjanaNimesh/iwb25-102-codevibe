...
const PatientRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      const res = await axios.get(`${API_BASE}/bloodrequests?hospital_id=${HOSPITAL_ID}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingRequest) {
        await axios.put(`${API_BASE}/bloodrequests/${editingRequest.request_id}?hospital_id=${HOSPITAL_ID}`, formData)
      } else {
        await axios.post(`${API_BASE}/bloodrequests/add?hospital_id=${HOSPITAL_ID}`, formData)
      }
      fetchRequests()
      resetForm()
    } catch (err) {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure you want to delete this request?')) {
      try {
        await axios.delete(`${API_BASE}/bloodrequests/${id}?hospital_id=${HOSPITAL_ID}`)
        fetchRequests()
      } catch (err) {
  ...
  const resetForm = () => {
    ...
    setFormData({ admission_number: '', blood_group: 'A+', units_required: 1, notes: '', request_status: 'Pending' })
    setShowModal(false)
  }
  ...
  {currentData.map((request) => (
    <tr key={request.request_id}><td className="px-6 py-4 text-left">{request.admission_number}</td> {/* left aligned */}
      <td className="px-6 py-4 text-center">
        <span className="px-2 py-1 bg-red-100 text-red-800 rounded-full">{request.blood_group}</span>
      </td>
      <td className="px-6 py-4 text-center">{request.units_required}</td> {/* right aligned */}
      <td className="px-6 py-4 text-center">{request.request_date}</td> {/* center aligned */}
      <td className="px-6 py-4 text-center">
        <span
          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(request.request_status)}`}
        >
          {request.request_status}
        </span>
      </td>
      <td className="px-6 py-4 text-center">{request.notes}</td> {/* left aligned */}
